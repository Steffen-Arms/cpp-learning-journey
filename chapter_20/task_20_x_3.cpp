...
int binarySearch(const int* array, int target, int min, int max)
{
    while (min <= max)
    {
        int midIndex{std::midpoint(min, max)};
...
int binraySearchRecursion(const int* array, int target, int min, int max)
{
    ...
    {
        return binraySearchRecursion(array, target, midIndex + 1, max);
    }
    else
    {
        return binraySearchRecursion(array, target, min, midIndex - 1);
    }
}
...
int main()
{
    ...
    for (int count{0}; count < numTestValues; ++count)
    {
        ...
        int index{
            binraySearchRecursion(array, testValues[count], 0,
